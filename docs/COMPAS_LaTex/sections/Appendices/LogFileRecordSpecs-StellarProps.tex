...
\stellarProperty{DRAWN\_KICK\_VELOCITY}{DOUBLE}{BaseStar::m\_SupernovaDetails.drawnKickVelocity}{Magnitude of natal~ kick velocity without accounting for fallback~(km~s$^{\minus{1}}$). \\ Supplied by user in grid file or drawn from distribution (default). \\ This value is used to calculate the actual kick velocity.}{Drawn\_Kick\_Velocity}{}
...
\stellarProperty{KICK\_VELOCITY}{DOUBLE}{BaseStar::m\_SupernovaDetails.kickVelocity}{Magnitude of natal kick velocity received during a supernova~(km~s$^{\minus{1}}$). Calculated using the drawn kick velocity.}{Applied\_Kick\_Velocity}{}
...
\constituentProperty{MASS\_TRANSFER\_CASE\_INITIAL}{INT}{BinaryConstituentStar::m\_MassTransferCase}{Indicator of mass transfer type when first RLOF occurs, if any. Printed as one of 
\begin{itemize}
    \item[]{NONE\tabto{3.5em}=\tabto{5em}0}
    \item[]{A\tabto{3.5em}=\tabto{5em}1}
    \item[]{B\tabto{3.5em}=\tabto{5em}2}
    \item[]{C\tabto{3.5em}=\tabto{5em}4}
    \item[]{D\tabto{3.5em}=\tabto{5em}8}
\end{itemize}
\ }{MT\_Case}{}
...
\stellarProperty{SUPERNOVA\_KICK\_VELOCITY\_MAGNITUDE\_RANDOM\_NUMBER}{DOUBLE}{BaseStar::m\_SupernovaDetails.kickVelocityRandom}{Random number for drawing the supernova kick velocity magnitude (if required). \\ Supplied by user in grid file, default = random number drawn from [0..1).}{SN\_Kick\_Magnitude\_Random\_Number}{}
