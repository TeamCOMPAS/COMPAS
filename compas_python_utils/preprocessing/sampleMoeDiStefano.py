...
def createParameterDistributionsAndSampler(testing=False):
    ...
    for i in range(0, numM1):
        ...
        for j in range(0, numlogP):
            ...
            fq[indsq] = fq[indq0p3]*np.power(qv[indsq]/0.3,gs)   # slope across 0.1 < q < 0.3
            ...
            if (np.power(10,mylogP) <= 2.):
                ...
                fe = np.power(ev, eta)
                e_max = 1 - np.power(0.5*np.power(10, mylogP), -2/3) # maximum eccentricity for given P
    ...
    if testing:
        ...
        nPoints = 150000
    M1 = np.linspace(0.08, 150, nPoints)
...
def createParameterGridfile(gridname, nSamples, m1Min=5, m1Max=100, testing=False):

    # Define the distributions and extract the sampler
    parameterSampler = createParameterDistributionsAndSampler(testing=testing)
...
def parse_args(cli_args=[]):
    ...
    parser.add_argument('--gridname', type=str, default=f'grid_moedistefano_samples{default_n}.txt', help='Name of the grid file to create')
    ...
    parser.add_argument('--testing', type=bool, default=False, help='Run with testing on')
