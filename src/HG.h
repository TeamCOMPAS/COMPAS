...
protected:
    ...
    void Initialise() {
        ...
        m_CoreMass    = std::min(m_Mass, std::max(CalculateCoreMassOnPhase(), MinimumCoreMass()));                                                                              //set core mass to be minimum core mass (only relevant if RetainCoreMassDuringCaseAMassTransfer() ) but no more than total mass
