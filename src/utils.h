...
namespace utils {
     ...
     * std::tuple<bool, E> GetMapKey(const M& p_Map, std::string p_Value, const E& p_Default)
     *
     ...
     * @return                                      Key corresponding to the value found, or the default if the  value was not found
     */
    template<typename M, typename E>
    std::tuple<bool, E> GetMapKey(std::string p_Value, const M& p_Map, const E& p_Default) {
        for (auto& it: p_Map)
    ...
    // JR: todo: document
    static std::tuple<bool, LOGFILE> GetLogfileDescriptorKey(std::string p_Value) {
        for (auto& it: LOGFILE_DESCRIPTOR)
            if (Equals(std::get<3>(it.second), p_Value)) return std::make_tuple(true, it.first);
