...
class pythonProgramOptions:
    ...
    populationPrinting = True

    randomSeedFileName = 'randomSeed.txt'
    if os.path.isfile(randomSeedFileName):
        random_seed = int(np.loadtxt(randomSeedFileName))
    else:
        random_seed = 45 # If you want a random seed, use: np.random.randint(2,2**63-1)

    compas_logs_output_override = os.environ.get('COMPAS_LOGS_OUTPUT_DIR_PATH')
    
    if (compas_logs_output_override is None):
        output = os.getcwd()
        output_container = 'TestSeed45'                 # names the directory to be created and in which log files are created.  Default in COMPAS is "COMPAS_Output"
    else:
    ...
    allow_touching_at_birth = True                                        # allow binaries that have stars touching at birth to evolve?
    ...
    remnant_mass_prescription = 'MULLERMANDEL'
    fryer_supernova_engine = 'DELAYED'
    black_hole_kicks = 'FALLBACK'
    kick_magnitude_distribution = 'MULLERMANDEL'
