...
protected:
    ...
    void Initialise() {
        ...
        m_IsSubChandrasekharTypeIa = false; // RTW: Should this be true at the start? NRS: Probably a confusing name, as this only flags whether there should be a SN Ia-like event at sub-Chandrasekhar mass value.
        m_Rejuvenate = false;
    ...
    // RTW: Might want to rename to m_IsSubChandrasekhar for clarity. Also, should this be the opposite, as in, go SN if !m_IsSubChandrasekharTypeIa ?
    // NRS: I agree with renaming, that is what causes confusion.
    // RTW: If the SN is AIC, you can copy the code in ONeWD.h
    bool    IsSupernova() const                                                           { return m_IsSubChandrasekharTypeIa; }     // Going supernova if mass and He shell are large enough
