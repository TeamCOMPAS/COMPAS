...
def plot_rates(save_dir, formation_rate, merger_rate, detection_rate, redshifts, chirp_masses, show_plot = False, mu0=0.035, muz=-0.23, sigma0=0.39, sigmaz=0., alpha=0):
    ...
    axes[0,0].text(0.7,0.8, "mu0 = %s \nmuz = %s \nsigma0 = %s \nsizmaz = %s\nalpha = %s"%(mu0, muz,sigma0, sigmaz, alpha), transform=axes[0,0].transAxes, size = fs) 

    for ax in axes.flatten():
        ax.tick_params(labelsize=0.9*fs)

    # Save and show :)
    plt.savefig(save_dir +'Rates'+"_mu0_%s_muz_%s_sigma0_%s_sigmaz_%s_alpha_%s"%(mu0, muz,sigma0, sigmaz, alpha)+'.png', bbox_inches='tight') 
    if show_plot:
        plt.show()
    else:
        plt.close()
...
if __name__ == "__main__":
    ...
    chirp_masses = (COMPAS.mass1*COMPAS.mass2)**(3./5.) / (COMPAS.mass1 + COMPAS.mass2)**(1./5.)
    plot_rates('./', formation_rate, merger_rate, detection_rate, redshifts, chirp_masses, show_plot = False, mu0=args.mu0, muz=args.muz, sigma0=args.sigma0, sigmaz=args.sigmaz, alpha=args.alpha)
    end_plot = time.time()

    
    print(detection_rate.shape)
    with open('rates_' + str(args.muz) + '_' + str(args.sigma0) + "_" + str(args.aSF) + "_" + str(args.dSF) + ".csv", 'w', newline = '') as csvFile:
        writer = csv.writer(csvFile)
        row = [str(args.muz), str(args.sigma0), str(args.aSF), str(args.dSF)]
        writer.writerow(row)
        row = [str(detection_rate.shape[0]), str(detection_rate.shape[1])]
        writer.writerow(row)
        for iRow in range(detection_rate.shape[0]):
            row = []
            for iCol in range(detection_rate.shape[1]): row.append(str(detection_rate[iRow][iCol]))
            writer.writerow(row)
    print('Sum det rate =', np.sum(detection_rate))

    print(chirp_masses.shape)
    with open('chirp_masses.csv', 'w', newline = '') as csvFile:
        writer = csv.writer(csvFile)
        row = []
        for iCol in range(chirp_masses.shape[0]): row.append(str(chirp_masses[iCol]))
        writer.writerow(row)
