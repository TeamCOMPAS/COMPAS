...
from physconst import DAY, MSUN, AU

from ..orbit import Pmt2a

"""
File obtained from Reinhold Willox on 20221008 with the following
comment:

The script is attached below. It was originally an IDL script from
Max Moe, converted into python by Mads Sorensen, then modified by me
to clean it up and produce a valid COMPAS input script. It will
sample both single stars, true binaries, and the innermost binaries
of hierarchical systems, but not tertiaries or other outer
components. True binaries and inner binaries are not
distinguished. Let me know if you run into any problems.

-----------------------------------------------------------------------

20221008 - Modfied by Alexander Heger as smaple inferface for
multistar.grid MC interface
...
The python code has been adopted by Mads SÃ¸rensen
-----------------------------------
Version history:
V. 0.1 2017/02/03
By Mads SÃ¸rensen
...
_log10 = np.log(10.)
def exp10(x):
    return np.exp(_log10 * x)

class MoeDiStefanoSampler(object):
    ...
    def __init__(self):
        ...
        print(f'[{self.__class__.__name__}] Generating interpolation table ...')

        starttime = time()

        # 0.8 < M1 < 40 (where we have statistics corrected for selection effects)
        M1v = exp10(np.linspace(0, 1, 101) * (np.log10(40.) - np.log10(0.8)) + np.log10(0.8))
        ...
        indlq = qv >= 0.3
        indsq = ~indlq
        indq0p3 = np.where(indlq)[0][0]

        # Loop through primary mass
        for i in tqdm(range(numM1)):
            myM1 = M1v[i]
            ...
            for j in range(numlogP):
                ...
                if (mylogP <= 1):
                    gl_6 = -0.5
                if ((mylogP > 1) and (mylogP <= 2)):
                    gl_6 = -0.5 - 0.9 * (mylogP - 1)
                if ((mylogP > 2) and (mylogP <= 4)):
                ...
                if (mylogP <= np.log10(2)):
                    # For P < 2 days, assume all systems are close to circular
                    # For adopted ev (spacing and minimum value), eta = -3.2 satisfies this
                    fe = ev**(-3.2)
                else:
                    fe = ev**eta
                    e_max = 1 - (exp10(mylogP) * 0.5)**(-2/3) # maximum eccentricity for given P
                    fe[ev >= e_max] = 0                       # set dist. = 0 for e > e_max
                    ...
                    ind = (0.8 * e_max <= ev) & (ev <= e_max)
                    ind_cont = np.where(ind)[0][0] - 1
                    fe[ind] = np.interp(ev[ind], [0.8 * e_max, e_max], [fe[ind_cont], 0])
                ...
                if(mylogP <= 1.5):
                    ...
                    probbin[j,i]=1 - 0.11 * (mylogP - 1.5)**1.43 * (myM1 * 0.1)**0.56
                if(probbin[j,i] <= 0):
                    probbin[j,i] = 0
        ...
        self.cumfM1 = cumfM1
        self.M1v = M1v
        self.cumPbindist = cumPbindist
        self.logPv = logPv
        self.cumedist = cumedist
        self.cumqdist = cumqdist
        self.ev = ev
        self.qv = qv

        print(f' [{self.__class__.__name__}] generated interpolation table in {time2human(time() - starttime)}.')

    # sampler function
    # Minimum primary mass to generate (must be > 0.080 Msun)

    def _parameterSampler(self, M1min=0.08, M1max=150, rng=None):

        # Value of primary mass CDF where M1 = M1min, M1Max
        cumf_M1min = np.interp(M1min, self.M1, self.cumfM1)
        cumf_M1max = np.interp(M1max, self.M1, self.cumfM1)

        # Select primary M1 > M1min from primary mass function
        myM1 = np.interp(cumf_M1min + (cumf_M1max - cumf_M1min) * rng.uniform(), self.cumfM1, self.M1)
        ...
        indM1 = np.where(abs(myM1 - self.M1v) == min(abs(myM1 - self.M1v)))[0]
        ...
        if(myrand < mybinfrac):
            ...
            mye = np.interp(rng.uniform(), self.cumedist[:, indlogP, indM1].flatten(), self.ev)
            ...
            myq = np.interp(rng.uniform(), mycumqdist, self.qv)
            ...
            myM2 = 0
            myP = np.inf # very wide
            mye = 0

        return myM1, myM2, myP, mye

    def parameterSampler(self, M1min=0.08, M1max=150, single=False, rng=None):
        while True:
            m1, m2, P, e = self._parameterSampler(M1min, M1max, rng)
            if single or m2 > 0:
                return m1, m2, P, e

    def __call__(self, M1min=0.08, M1max=150, single=False, size=100, rng=None):
        """
        return array with M1 (MSUN), M2 (MSUN), P (DAY), e
        """
        print(f' [{self.__class__.__name__}] Generating {size} binaries...')
        ...
        vv = np.array([self.parameterSampler(M1min, M1max, False, rng=rng) for _ in tqdm(range(size))]).T

        print(f' [{self.__class__.__name__}] data generated in {time2human(time() - starttime)}.')

        return vv

class MoeDiStefanoMQAE(MoeDiStefanoSampler):

    def __call__(self, *args, **kwargs):
        """
        return array with M=M1+M2 (Msun), q = M2/M1, a (AU), e
        """
        vv =  super().__call__(*args, **kwargs)

        vv[0:2] = vv[0] + vv[1], vv[1] / vv[0]
        vv[2] = Pmt2a(vv[2] * DAY, vv[0] * MSUN) / AU
